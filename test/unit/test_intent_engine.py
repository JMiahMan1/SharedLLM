...
def engine():
    ...
    with patch("gateway.intent_engine.json.load", return_value={"turn_on": ["turn on light"]}):
...
async def test_gateway_skips_llm_on_high_confidence(mocker):
    from gateway.main import chat_handler
    ...
    mocker.patch("gateway.main.get_long_term_memory", new_callable=AsyncMock, return_value="")
    
    # Mock the global engine object in main.py
    mocker.patch("gateway.main.engine.classify", return_value=("turn_on", 0.98))
    mocker.patch("gateway.main.engine.should_bypass_llm", return_value=True)
    
    mock_exec = mocker.patch("gateway.main.execute_command", new_callable=AsyncMock, return_value={"status": "SUCCESS", "message": "Lights on"})
    mock_ollama = mocker.patch("gateway.main.call_ollama", new_callable=AsyncMock)
