    ...
    def update_container(self, container_info: ContainerInfo):
        ...
        for hostname in container_info.hostnames:
            if hostname.startswith("."):
                # Wildcard pattern
                self.wildcards.append((hostname.lstrip("."), container_info))
            else:
                self.hostname_map[hostname] = container_info
...
class DNSServer:
    ...
    def __init__(self, registry: DNSRegistry, port: int = DNS_PORT):
    ...
    async def on_docker_event(action: str, container_id: str, attrs: dict):
        ...
        if action == "start":
            ...
            if container:
                ...
                logging.info(f"Added container: {container.name} ({container.ip_address})")
        elif action == "stop":
