        ...
        try:
            ...
            if lookup_action in action_map:
                ...
                if lookup_action == "ttsrequest" and isinstance(payload, dict):
                    ...
                    # execution schema REQUIRES user_context for workspace-scoped
                    # calls (tts, workspace_file_read). Synthesize one from creds.
                        ...
                        try:
                            async with shared_http_client() as _tc:
                                _tf_resp = await _tc.post(
                                    ...
                                    json={"path": _tts_text_file, "workspace_id": _tts_ws, "user_context": _tts_uc},
                                    headers={"X-Internal-Secret": INTERNAL_SECRET},
                    ...
                    if not _tts_text:
                        ...
                        try:
                            ...
                            # never reads it — so pass the synthesized one from above.
