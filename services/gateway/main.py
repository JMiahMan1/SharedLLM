...
async def _build_raven_system_prompt(query: str) -> str:
    ...
    if any(token in (query or "").lower() for token in TTS_SIGNALS):
        tts_section = (
            ...
            "period — the pause improves delivery); ensure numbers, abbreviations and "
            "scripture references read naturally aloud (e.g. expand `John 3:16` as "
            "`John chapter 3, verse 16`). (2) DO NOT read those text files into context — "
            "use `text_file` to point at them. (3) In a SINGLE turn, emit a JSON ARRAY of "
            "TTSRequest calls, one per chapter, each with its own `text_file` and a "
            "distinct `file_path` (e.g. `chapter_01.wav` ... `chapter_08.wav`). (4) After "
