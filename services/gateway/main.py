...
async def _build_raven_system_prompt(query: str) -> str:
    ...
    if any(token in (query or "").lower() for token in TTS_SIGNALS):
        tts_section = (
            ...
            "Prepare the narration text as one clean `.txt` file PER chapter/section "
            ...
            "material; keep headings as their own lines (e.g. `Chapter One.` with a "
            "period — the pause improves delivery). (2) DO NOT write a Python/JS script "
            "to normalize pronunciation and DO NOT expand scripture references or years "
            "yourself — the TTS engine already reads Bible verses correctly (e.g. "
            "`John 3:16` is spoken as `John chapter 3, verse 16`, `2 Timothy 3:16-17` as "
            "`Second Timothy chapter 3, verses 16 through 17`) and years/dates correctly "
            "(e.g. `1995` as `nineteen ninety-five`, `1400 BC` as `fourteen hundred "
            "B.C.`). Leave verses and years exactly as they appear in the source. (3) DO "
            "NOT read those text files into context — "
            "use `text_file` to point at them. (4) In a SINGLE turn, emit a JSON ARRAY of "
            ...
            "exhausts your window and causes failures."
