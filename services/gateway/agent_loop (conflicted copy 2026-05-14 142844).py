...
async def AgentLoop(query: str, selected_model: str, full_system: str, short_term: list, rag_user: str, creds: ResolvedCredentials, mission_id: Optional[int] = None) -> Any:
    ...
    async def stream_event(event_type: str, data: str):
        ...
        r = await get_stream_redis()
        try:
            msg_str = json.dumps(msg_obj)
            await r.rpush(f"raven:mission:history:{mission_id}", msg_str)
            await r.expire(f"raven:mission:history:{mission_id}", 86400)
            await r.publish(f"raven:mission:stream:{mission_id}", msg_str)
        except Exception as e:
