...
def test_workspace_crud(client: TestClient):
    ...
    resp = client.post("/workspaces", json=ws_data, headers={"X-Internal-Secret": os.getenv("INTERNAL_SECRET")})
    assert resp.status_code == 200
    assert resp.json()["workspace"]["id"] == "test_ws"

    # 2. Read (List)
    resp = client.get("/workspaces", headers={"X-Internal-Secret": os.getenv("INTERNAL_SECRET")})
    assert resp.status_code == 200
    data = resp.json()
    assert any(ws["id"] == "test_ws" for ws in data["workspaces"])

    # 3. Update
    resp = client.patch("/workspaces/test_ws", json={"display_name": "Updated Name"}, headers={"X-Internal-Secret": os.getenv("INTERNAL_SECRET")})
    assert resp.status_code == 200
    assert resp.json()["workspace"]["display_name"] == "Updated Name"

    # 4. Delete
    resp = client.delete("/workspaces/test_ws", headers={"X-Internal-Secret": os.getenv("INTERNAL_SECRET")})
    assert resp.status_code == 200
