...
| **`execution`** | **Greatly Expanded.** Houses 35+ tool handlers. Dedicated TV Brand Handlers (`roku.py`, `samsung.py`, `webos.py`, `android_tv.py`) with brand-specific transport maps. Roku music uses ECP + MA sibling-delegation. Android TV video delegates to Cast sibling via `_find_cast_sibling()` (capability-based detection). A **10-strategy Device Discovery Pipeline** (`device_discovery.py`) and persistent **SQLite Device Registry** (`device_registry.py`) discover device IPs (cache → HA → HomeKit diagnostics → ARP → SNMP → mDNS → SSDP → port scan). Credentials resolved from Identity at runtime — `.env` is seed-only. YouTube search uses `yt-dlp ytsearch:1`. | No critical blockers. Subnet detection is now dynamic. | **Capability Widgets.** Powers Media, Timer, Notes, and Smart Home toggle widgets. |
