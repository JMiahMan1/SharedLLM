...
def extract_text_content(file_path: str) -> Optional[str]:
    ...
    if not ext:
        ...
        logger.warning("File extension is empty or non-existent for extraction at %s.", file_path)
        return None
...
def list_files_webdav(base_url: str, all_found_files, seen=None):
    ...
    try:
        ...
        for r in root.findall('d:response', NAMESPACES):
            ...
            current_relative_dir = urlparse(base_url).path.strip('/').replace(user_prefix.strip('/'), '', 1).strip('/')
            
            if relative_path == "" or relative_path == current_relative_dir:
                continue
            
            is_collection = propstat.find('d:prop/d:resourcetype/d:collection', NAMESPACES) is not None
            ...
            if is_collection:
                ...
                subdir_url = urljoin(NEXTCLOUD_URL, f"/remote.php/dav/files/{NEXTCLOUD_USER}/{relative_path}/")
                logger.info("Entering directory: %s (WebDAV URL: %s)", relative_path, subdir_url)
...
def ingest_nextcloud_files():
    ...
    logger.info("Starting Nextcloud ingestion process (PRODUCTION MODE: SSL VERIFICATION DISABLED).")
    ...
    try:
        ...
        logger.error("CRITICAL ERROR: Failed to create temporary directory %s. Check Docker volume permissions: %s", INGESTION_TEMP_DIR, e)
        sys.exit(1)
    ...
    logger.info("Finished file discovery. Found a total of %d files to check/ingest.", len(files))
    
    # 3. Process Files One-by-One
    for i, relative_path in enumerate(files, start=1):
        file_url = urljoin(NEXTCLOUD_URL, f"/remote.php/dav/files/{NEXTCLOUD_USER}/{relative_path}")
        ...
        try:
            ...
            existing_docs = vectordb._collection.get(
                ids=[doc_id], 
                include=[] 
            )
            if existing_docs['ids']:
                logger.info("[%d/%d] Skipping: %s - Already ingested.", i, len(files), relative_path)
                continue
            
            logger.info("[%d/%d] Ingesting: %s (Type: %s)", i, len(files), relative_path, ext.upper().strip('.'))
            ...
            temp_file_path = os.path.join(INGESTION_TEMP_DIR, f"{unique_id_prefix}_{file_name_with_ext}")

            # Stream the download directly to the path with the extension
            # SSL FIX: Added verify=False
            with requests.get(file_url, auth=(NEXTCLOUD_USER, NEXTCLOUD_PASS), stream=True, verify=False, timeout=(30, 180)) as r:
                r.raise_for_status()
            ...
            if file_size == 0:
                 raise IOError(f"Downloaded file {relative_path} is 0 bytes. Check Nextcloud permissions/WebDAV status.")
            ...
            if not text_content or not text_content.strip():
                logger.warning("Extracted content empty or failed for file: %s", relative_path)
                continue
            
            # --- Ingestion to ChromaDB ---
            doc = Document(
                page_content=text_content, 
                metadata={"source": "nextcloud", "path": relative_path, "type": ext.strip('.')}
            )

            logger.info("DEBUG: Prepared document with ID %s. Content length: %d. Attempting add to Chroma...", doc_id, len(doc.page_content))
            
            vectordb.add_documents(documents=[doc], ids=[doc_id])
            INGESTED_COUNT += 1
            logger.info("SUCCESS: Document for %s added and indexed. Total added in this run: %d", relative_path, INGESTED_COUNT) 
            ...
            logger.error("Unexpected error processing %s: %s", relative_path, e, exc_info=True)
        finally:
            # --- CRITICAL CLEANUP ---
    ...
    # --- Final Persistence ---
    vectordb.persist() 
    logger.info("Nextcloud ingestion complete. Total Documents added in this run: %d. Final document count: %d", INGESTED_COUNT, vectordb._collection.count())
