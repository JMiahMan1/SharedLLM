...
from logic.music_assistant_ops import (
    tool_list_playlists,
    tool_list_radio,
    tool_music_search,
)
...
async def handle_timer_add(
    query: str, user_creds: dict, model: str, params: dict = None, **kwargs
):
...
async def handle_media_previous(
    query: str, user_creds: dict, params: dict = None, **kwargs
):
    return await handle_media_command(
        "media_previous",
...
async def handle_remote_command(
    ...
    if not entity_id:
        return {
            "status": "FAILURE",
            "message": "No entity specified for remote command.",
        }

    return await tool_remote_command(
        cmd, entity_id, user_creds, GlobalResources.redis_client
    )
...
async def handle_note_add(query: str, params: dict = None, **kwargs):
    p = params or {}
    res = await tool_note_add(
        p.get("title", "New Note"),
        p.get("content", query),
...
async def handle_note_append(query: str, params: dict = None, **kwargs):
    p = params or {}
    res = await tool_note_append(
        p.get("title", "Shopping List"), p.get("content", query)
    )
    return {
        "status": "SUCCESS" if res.get("status") == "success" else "FAILURE",
        "message": res.get("msg", ""),
...
async def handle_note_read(params: dict = None, **kwargs):
    ...
    return {
        "status": "SUCCESS" if "Note Content" in res else "FAILURE",
        "message": res,
        "service": "note_read",
    }
...
async def handle_note_delete(params: dict = None, **kwargs):
    ...
    return {
        "status": "SUCCESS" if "deleted" in res.lower() else "FAILURE",
        "message": res,
        "service": "note_delete",
    }
...
async def handle_intent_learn(params: dict = None, **kwargs):
    res = (
        f"Cannot learn '{params.get('phrase', '')}' with current prompt context."
        if params
        else "Cannot learn phrase."
    )
...
async def handle_music_list(query: str, user_creds: dict, **kwargs):
    q_low = query.lower()
    if "radio" in q_low or "station" in q_low:
        return await tool_list_radio(query, user_creds, GlobalResources.redis_client)
    else:
        return await tool_list_playlists(
            query, user_creds, GlobalResources.redis_client
        )


@ActionDispatcher.register("music_search")
async def handle_music_search(query: str, user_creds: dict, **kwargs):
    return await tool_music_search(query, user_creds, GlobalResources.redis_client)
