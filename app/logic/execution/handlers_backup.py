...
async def _find_remote_entity_for_device(entity_id: str) -> str:
    """Find the remote entity for a given device using group lookup (via find_group_sibling)"""
    from app.domains.media.devices import find_group_sibling
    
    # Define match function for remote domain
    def is_remote(metadata):
        candidate_id = metadata.get("entity_id")
        return candidate_id and candidate_id.split('.')[0] == "remote"

    # Try group lookup first
    remote_entity = await find_group_sibling(entity_id, is_remote)
    
    if remote_entity:
        log.info(f"[REMOTE LOOKUP] Found remote entity via group: {remote_entity}")
