...
DEFAULT_PHRASES = {
    ...
    "toggle": ["toggle", "flip", "change state", "switch state"],
    "play_media": [
        "play",
        "resume",
        "start music",
...
class IntentEngine:
    ...
    async def load(self):
        ...
        if not GlobalResources.embedding_model:
            log.warning(
                "Embedding model not loaded. Intent Engine running in Keyword Mode only."
            )
            return
        ...
        try:
            ...
            vectors = await run_blocking(
                lambda: GlobalResources.embedding_model.embed_documents(all_texts)
            )
    ...
    async def classify(
        ...
        try:
            query_vec = await run_blocking(
                lambda: GlobalResources.embedding_model.embed_query(query)
            )
            ...
            log.debug(
                f"Intent Low Confidence: '{query}' -> Best: '{intent}' ({best_score:.2f})"
            )
    ...
    async def learn(self, phrase: str, intent: str):
        if intent not in self.phrase_map:
            return False
