# media troubleshooting prompt — Template

This is a fill-in-the-blank template. It is tracked in git as a reference.
The actual prompt content goes in `prompts/media_troubleshooting_prompt.md` (git-ignored).
...
1. **Copy and populate:**
   ...
   ```bash
   cp prompts/media_troubleshooting_prompt.md.sample prompts/media_troubleshooting_prompt.md
   ```

   Then open `prompts/media_troubleshooting_prompt.md` and replace the `[ ... ]` placeholders below
...
3. **Verify:** After seeding, check the prompt loaded:
   ...
   docker exec sharedllm_identity sqlite3 /data/identity.db \
     "SELECT length(value) FROM globalsetting WHERE key='media_troubleshooting_prompt';" \
...
- **Never commit `media_troubleshooting_prompt.md`** — it contains production prompt content
- Keep `media_troubleshooting_prompt.md.sample` as a lightweight reference template only
...
| Prompt too short/empty | Verify `media_troubleshooting_prompt.md` has actual content (not just placeholders) |
