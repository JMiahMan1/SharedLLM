# single turn tool guide — Template

This is a fill-in-the-blank template. It is tracked in git as a reference.
The actual prompt content goes in `prompts/single_turn_tool_guide.md` (git-ignored).
...
1. **Copy and populate:**
   ...
   ```bash
   cp prompts/single_turn_tool_guide.md.sample prompts/single_turn_tool_guide.md
   ```

   Then open `prompts/single_turn_tool_guide.md` and replace the `[ ... ]` placeholders below
...
3. **Verify:** After seeding, check the prompt loaded:
   ...
   docker exec sharedllm_identity sqlite3 /data/identity.db \
     "SELECT length(value) FROM globalsetting WHERE key='single_turn_tool_guide';" \
...
- **Never commit `single_turn_tool_guide.md`** — it contains production prompt content
- Keep `single_turn_tool_guide.md.sample` as a lightweight reference template only
...
| Prompt too short/empty | Verify `single_turn_tool_guide.md` has actual content (not just placeholders) |
