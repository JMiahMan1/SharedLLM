# raven reflection prompt — Template

This is a fill-in-the-blank template. It is tracked in git as a reference.
The actual prompt content goes in `prompts/raven_reflection_prompt.md` (git-ignored).
...
1. **Copy and populate:**
   ...
   ```bash
   cp prompts/raven_reflection_prompt.md.sample prompts/raven_reflection_prompt.md
   ```

   Then open `prompts/raven_reflection_prompt.md` and replace the `[ ... ]` placeholders below
...
3. **Verify:** After seeding, check the prompt loaded:
   ...
   docker exec sharedllm_identity sqlite3 /data/identity.db \
     "SELECT length(value) FROM globalsetting WHERE key='raven_reflection_prompt';" \
...
- **Never commit `raven_reflection_prompt.md`** — it contains production prompt content
- Keep `raven_reflection_prompt.md.sample` as a lightweight reference template only
...
| Prompt too short/empty | Verify `raven_reflection_prompt.md` has actual content (not just placeholders) |
