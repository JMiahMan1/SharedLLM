# assistant system instruction — Template

This is a fill-in-the-blank template. It is tracked in git as a reference.
The actual prompt content goes in `prompts/assistant_system_instruction.md` (git-ignored).
...
1. **Copy and populate:**
   ...
   ```bash
   cp prompts/assistant_system_instruction.md.sample prompts/assistant_system_instruction.md
   ```

   Then open `prompts/assistant_system_instruction.md` and replace the `[ ... ]` placeholders below
...
3. **Verify:** After seeding, check the prompt loaded:
   ...
   docker exec sharedllm_identity sqlite3 /data/identity.db \
     "SELECT length(value) FROM globalsetting WHERE key='assistant_system_instruction';" \
...
- **Never commit `assistant_system_instruction.md`** — it contains production prompt content
- Keep `assistant_system_instruction.md.sample` as a lightweight reference template only
...
| Prompt too short/empty | Verify `assistant_system_instruction.md` has actual content (not just placeholders) |
