...
if ssh $SSH_OPTS "$HOST" << EOF
    ...
    GATEWAY_CONTAINER=$(docker ps --format '{{.Names}}' | grep '_sharedllm_gateway' | head -1)
    GATEWAY_CONTAINER=${GATEWAY_CONTAINER:-$(docker ps --format '{{.Names}}' | grep 'gateway' | head -1)}
    GATEWAY_CONTAINER=${GATEWAY_CONTAINER:-sharedllm_gateway}
